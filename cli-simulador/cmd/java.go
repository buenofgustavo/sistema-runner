...
func (j *JavaInvoker) Run(jarPath string, args []string) (string, error) {
	...
	if err != nil {
		...
		if provisionErr != nil {
			return "", fmt.Errorf("java não encontrado e falha ao provisionar: %w (erro original: %v)", provisionErr, err)
		}
		javaPath, err = findJava()
		if err != nil {
	...
	if _, err := os.Stat(jarPath); err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return "", fmt.Errorf("arquivo jar não encontrado: %s", jarPath)
		}
		return "", fmt.Errorf("não foi possível acessar o jar: %w", err)
	}
...
func checkJavaVersion(javaPath string) error {
	...
	if idx == -1 {
		return fmt.Errorf("não foi possível identificar a versão do Java na saída: %s", versionStr)
	}
	...
	if len(parts) == 0 {
		return fmt.Errorf("versão inválida do Java: %s", fullVersion)
	}
	...
	if err != nil {
		return fmt.Errorf("falha ao analisar versão do Java '%s': %w", fullVersion, err)
	}

	if major < 21 {
		return fmt.Errorf("versão do Java %d é inferior a 21. Requer versão >= 21", major)
	}
...
func findJava() (string, error) {
	// 1. Buscar em ~/.hubsaude/jdk/ primeiro
	if homeDir, err := os.UserHomeDir(); err == nil {
		localJreDir := filepath.Join(homeDir, ".hubsaude", "jdk")
	...
	return "", errors.New("java 21+ não encontrado. Instale o Java 21 ou superior")
}

...
func EnsureJDK() error {
	...
	// Não encontrado, vamos provisionar na pasta ~/.assinatura/jre
	homeDir, err := os.UserHomeDir()
	if err != nil {
		return fmt.Errorf("nao foi possivel obter diretorio do usuario: %w", err)
	}

	targetDir := filepath.Join(homeDir, ".assinatura", "jre")
	...
	tempFile, err := os.CreateTemp("", "jre-download-*"+format)
	if err != nil {
...
func getAdoptiumURL() (string, string, error) {
	...
	if arch != "amd64" {
		// Fallback ou aviso, mas tentamos prosseguir
		fmt.Printf("Aviso: arquitetura %s detectada, baixando versao x64/amd64 por padrão\n", arch)
	}
	...
	default:
		return "", "", fmt.Errorf("sistema operacional nao suportado para provisionamento automatico: %s", osType)
	}
}
